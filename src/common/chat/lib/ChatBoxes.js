...
$.widget('ui.chatbox', {
  options: {
    ...
    boxClosed(id) {}, // called when the close icon is clicked
    boxManager: {
      ...
      highlightBox() {
        ...
        self.elem.uiChatbox.effect('bounce', { times: 2 }, 300, () => {
  ...
  _create() {
    ...
    const uiChatboxTitlebar = (self.uiChatboxTitlebar = $('<div></div>'))
      .addClass(
        `${
          ...
          // 'ui-corner-top ' +
          'ui-chatbox-titlebar '
        }${self.options.status} ui-dialog-header`, // take advantage of dialog header style
      )
      .click((event) => {
        self.toggleContent(event);
      })
      .appendTo(uiChatbox);
    const uiChatboxTitle = (self.uiChatboxTitle = $('<span></span>'))
      .html(title)
      .appendTo(uiChatboxTitlebar);
    var uiChatboxTitlebarClose = (self.uiChatboxTitlebarClose = $(
      ...
      .appendTo(uiChatboxTitlebar);
    const uiChatboxTitlebarCloseText = $('<i></i>')
      .addClass('fa fa-times')
      .appendTo(uiChatboxTitlebarClose);
    var uiChatboxTitlebarMinimize = (self.uiChatboxTitlebarMinimize = $(
      ...
      .appendTo(uiChatboxTitlebar);
    const uiChatboxTitlebarMinimizeText = $('<i></i>')
      .addClass('fa fa-minus')
      .appendTo(uiChatboxTitlebarMinimize);
    // content
    const uiChatboxContent = (self.uiChatboxContent = $(
      `<div class="${self.options.alertshow}"><span class="alert-msg">${self.options.alertmsg}</span></div>`,
    ))
    ...
    const uiChatboxInput = (self.uiChatboxInput = $('<div></div>'))
      ...
      .appendTo(uiChatboxContent);
    var uiChatboxInputBox = (self.uiChatboxInputBox = $(
      '<textarea></textarea>',
    ))
...
$.fn.emoticonize = function (options) {
  const opts = $.extend({}, $.fn.emoticonize.defaults, options);
  ...
  for (let j = twoCharacterEmoticons.length - 1; j >= 0; --j) {
  ...
  for (const emoticon in specialEmoticons) {
  ...
  return this.not(exclude).each(function () {
    ...
    for (const emoticon in specialEmoticons) {
      const specialCssClass = `${cssClass} ${specialEmoticons[emoticon].cssClass}`;
      container.html(
        container
          .html()
          .replace(
    ...
    $(threeCharacterEmoticons).each(function () {
      container.html(
        container.html().replace(this, `$1<span class='${cssClass}'>$2</span>`),
      );
    });
...
const boxClosedCallback = function (id) {
  ...
  if (idx !== -1) {
    ...
    const diff = chatbox_config.width + chatbox_config.gap;
...
var dispatch = function (id, user, msg) {
  ...
  if ($('#chatlog').length) {
    $('#chatlog')
      .append(
        `You said to <b>${user.first_name} ${user.last_name}:</b> ${msg}<br/>`,
      )
